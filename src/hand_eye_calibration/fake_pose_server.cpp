...
int main(int argc, char **argv)
{
  ...
  if( !n.getParam("hec/fake_pose/world_to_robot/rotation/x",t_RW.rotation.x() ) )
    t_RW.rotation.x() = uniform_rv(rng);
  if( !n.getParam("hec/fake_pose/world_to_robot/rotation/y",t_RW.rotation.y() ))
    t_RW.rotation.y() = uniform_rv(rng);
  if( !n.getParam("hec/fake_pose/world_to_robot/rotation/z",t_RW.rotation.z() ))
    t_RW.rotation.z() = uniform_rv(rng);
  if( !n.getParam("hec/fake_pose/world_to_robot/rotation/w",t_RW.rotation.w() ))
    t_RW.rotation.w() = uniform_rv(rng);
  
  t_RW.rotation.normalize();
  ...
  if( !n.getParam("hec/fake_pose/eye_to_hand/rotation/x",t_HE.rotation.x() ))
    t_HE.rotation.x() = uniform_rv(rng);
  if( !n.getParam("hec/fake_pose/eye_to_hand/rotation/y",t_HE.rotation.y() ))
    t_HE.rotation.y() = uniform_rv(rng);
  if( !n.getParam("hec/fake_pose/eye_to_hand/rotation/z",t_HE.rotation.z() ))
    t_HE.rotation.z() = uniform_rv(rng);
  if( !n.getParam("hec/fake_pose/eye_to_hand/rotation/w",t_HE.rotation.w() ))
    t_HE.rotation.w() = uniform_rv(rng);
  
  t_HE.rotation.normalize();
  ...
  ros::ServiceServer eye_position_server = n.advertiseService("hec_eye_pose", &eyePositionService );
  ros::ServiceServer hand_position_server = n.advertiseService("hec_hand_pose", &handPositionService );
